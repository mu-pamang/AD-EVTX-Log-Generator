# Windows Event Log Generator v10
# Fixed: Stop W32Time before Set-Date
...
$REAL_TIME    = Get-Date
$TARGET_TOTAL = 10000
$DAILY_TARGET = [int]($TARGET_TOTAL / $DAYS_BEFORE)
...
Write-Host "======================================"
Write-Host " Event Log Generator v10"
Write-Host " Target: 10,000 logs (fast mode)"
Write-Host " Period: 05-14 ~ 05-21"
Write-Host " Estimated time: 1~2 hours"
Write-Host "======================================"

# [1] W32Time 서비스 중지 (시간 자동 동기화 방지)
Write-Host "`n[1] Stopping W32Time service..."
try {
    Stop-Service W32Time -Force
    Write-Host "  Done"
} catch {
    Write-Host "  Failed - continuing anyway"
}

# [2] 기존 로그 클리어
Write-Host "`n[2] Clearing existing logs..."
try {
...
# [3] 날짜별 로그 생성
Write-Host "`n[3] Generating logs..."
$total_count = 0
...
# [4] 시스템 시간 복원
Write-Host "`n[4] Restoring system time..."
Set-Date -Date $REAL_TIME
Start-Sleep -Seconds 2
Write-Host "  Restored: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"

# [5] W32Time 서비스 재시작 및 동기화
Write-Host "`n[5] Restarting W32Time service..."
try {
    Start-Service W32Time
    w32tm /resync /force
    Write-Host "  Done"
} catch {
    Write-Host "  Failed - please run manually: Start-Service W32Time"
}

# [6] 최종 로그 수 확인
Write-Host "`n[6] Final log count..."
$final_count = (Get-WinEvent -LogName Security | Measure-Object).Count
Write-Host "  Security log total: $final_count"

# [7] 흔적 삭제
Write-Host "`n[7] Cleaning up..."
$dl = "C:\Users\Administrator\Downloads"
foreach ($ver in @("","v2","v3","v4","v5","v6","v7","v8","v9","v10")) {
